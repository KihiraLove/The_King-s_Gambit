...
public class BoardManager : MonoBehaviour
{
    ...
    private void MovePiece(int x, int y, int z)
    {
        ...
        var boardCoordinate = getBoardCoordinates(y, z);
        if (selectedPiece.GetType() == typeof(King) &&
            (x == (int) selectedPiece.position.x - 2 || x == (int) selectedPiece.position.x + 2) &&
            selectedPiece.roundMoved == 0 && allowedMoves[x, boardCoordinate[0], boardCoordinate[1]])
        {
            Castle(x);
            return;
        }
        ...
        {
            var c = Pieces[x, boardCoordinate[0], boardCoordinate[1]];
            ...
            Pieces[(int) selectedPiece.position.x, (int) selectedPiece.position.y, (int) selectedPiece.position.z] =
                null;
    ...
    private void EndGame()
    {
        ...
        foreach (var go in activeChessPieces) Destroy(go);
    ...
    private void UpdateSelection()
    {
            ...
            LayerMask.GetMask("ChessBoards")))
        {
            ...
            else if (tempY > 0) selectiony = SECOND_BOARD_HEIGHT;
        }
        else
        {
            selectionx = -1;
    ...
    private void SpawnPiece(int index, int x, int y, int z, bool forward = true)
    {
        ...
        var piece = Instantiate(chessPiecesPrefabs[index], new Vector3(x, y, z), orientation);
    ...
    private void InitPieces()
    {
        ...
        //Pawns
        for (var i = 0; i < 8; i++) SpawnPiece(5, i, SECOND_BOARD_HEIGHT, 6 + SECOND_BOARD_OFFSET);
        ...
        SpawnPiece(10, 6, 0, 0);
        //Pawns
        for (var i = 0; i < 8; i++) SpawnPiece(11, i, 0, 1);
    }
}